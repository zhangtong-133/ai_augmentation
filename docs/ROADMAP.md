...
已完成知识库闭环、工具、长期记忆、对话/用户消息页面和 Redis 快照缓存。显式回复已提供[持久化请求、HTTP 与内置夹具执行器](design/sprint-3-model-replies.md)，下一步接回复页面；默认关闭，不调用真实模型。阶段边界见 [Sprint 3 设计](design/sprint-3-tools-memory-scheduler.md)。
