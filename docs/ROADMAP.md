...
已完成知识库闭环、受限工具、手动长期记忆、Redis 适配器及对话元数据 API。下一步实现消息幂等与删除并发保护，再接入 Redis；当前尚无消息或聊天入口。阶段边界见 [Sprint 3 设计](design/sprint-3-tools-memory-scheduler.md)。
...
- [ ] 消息接口与 Redis 接入（持久化幂等、删除并发保护及失败恢复）
- [ ] MCP adapter 的首个只读工具
