...
登录后的「知识检索与问答」支持「检索资料」和「生成引用答案」，仅在显式提交时调用 API。问题限制 1–1000 个字符，检索最多展示 5 个核验片段；问答展示答案、引用编号、文档标题、来源和可展开的原文片段。答案、来源和片段均按纯文本渲染，不执行 HTML、Markdown 或来源链接。得分不是置信概率，引用有效不保证答案语义正确。

页面分别提示无匹配片段、证据不足、未启用、登录失效、限流和服务故障；无效成功响应不会展示成答案或空结果。新请求清空旧结果并禁止重复提交，不自动索引或重试。客户端最多等待 65 秒；「停止等待」只中断客户端，不能承诺撤销服务端处理或费用。请求取消、退出或切换账户后忽略迟到响应；不写浏览器存储，刷新后清空问题与答案。

`make browser-test-index` 使用已有真实 PostgreSQL/Qdrant 和本地模型夹具，验证两个入口的索引→检索→答案→引用核对及账户隔离；响应夹具额外覆盖错误分流、恶意文本、无效引用响应和取消后的状态清理。不调用付费模型，也不把夹具结果作为模型事实准确率评估。

验收：Rust 全量检查、前端 lint/typecheck/build、Compose 配置和差异检查通过。最终联合验收通过 3 项 PostgreSQL 测试、1 项 Qdrant 测试、双入口 HTTP/重启持久化及 16 项浏览器测试；2 项公网用例按配置跳过，未运行 MinIO 专项或真实模型。桌面与窄屏截图已检查。首次运行的一次窄屏检索服务错误未复现，根因尚未确认；已增加 HTTP 状态与脱敏错误码断言，未放宽检查或自动重试。隔离测试资源已清理。
...
后端各步已更新路线图、运行 Rust 和前端检查，并使用隔离 PostgreSQL/Qdrant 与本地模型夹具验收。真实付费模型不在自动验收范围；页面交互在上述后续迭代交付。
