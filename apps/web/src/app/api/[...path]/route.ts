...
async function proxy(request: NextRequest, context: { params: Promise<{ path: string[] }> }) {
  ...
  const indexJob = (request.method === "GET" || request.method === "POST") && /^documents\/[a-f0-9-]{36}\/index-jobs$/i.test(endpoint);
  if (!allowed.includes(endpoint) && !documentDetail && !documentIndex && !indexJob) {
